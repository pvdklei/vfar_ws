...
% \\ \\
\subsection{The line detection pipeline}
...
After preprocessing, the Canny edge detection, a gradient-based method that also includes noise reduction, non-maximum suppression and hysteresis thresholding, is applied to the image.
...
The implementation was made in Python using OpenCV functions. Beside the binarizing threshold, there were a few hyperparameters introduced, as required form the HoughLinesP function: a minimum number of votes (70 in our case), the minimum line length (50 for us), and the maximum line gap (20 for us). The code can run equally well for a single image as well as a set of images (representing the frames from a video recording).

\subsection{Performance on five frames}
Figure \ref{fig:lines_v1} shows five different detection results using this pipeline, displaying the original images beside the detected lines by our implementation (second column). Overall, Canny manages to find the ceiling lights without many other irrelevant edges, though it fails to detect the lines curved by the fish-eye lens (i.e.: if the robot is positioned sideways, like in the last row). Another issue is that each ceiling light is delimited by two lines instead of one.
The third image column concerns Section 2.4 which compares Canny with a different edge detection algorithm.
...
This session is theoretical and not actually implemented due to time constraints and limited robot access.
...
\section{Conclusion}
This report designed and implemented a line detection pipeline for a UGV Rover. Using grayscaling, followed by binarization, dilatation, Canny edge detection and Hough Line Transform proved to be an effective method for detecting ceiling lines.
